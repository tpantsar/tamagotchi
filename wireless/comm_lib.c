...
void Send6LoWPAN(uint16_t DestAddr, uint8_t *ptr_Payload, uint8_t u8_length)
{
	...
	for (u32_cnt = 0; u32_cnt < APP_ADVERTISE_PERIOD; u32_cnt++)
	{ // SW timer
		if (u8_TXd_Flag)
		{ // in case if defined - leave the loop once radio informs about TX end
			u8_TXd_Flag = 0;
			break;
		}
	}
...
CC2650_RXEntry_Decode(uint8_t *ptr_DataStart, CWC_CC2650_RX_Entry_struct_t *ptr_CC2650_RXQueueStruct)
{
	uint8_t *ptr_u8;
	uint8_t u8_length;
	if ((ptr_CC2650_RXQueueStruct == NULL) || (ptr_DataStart == NULL))
		return 0;
...
CC2650_RXEntry_Release(uint8_t *ptr_Data)
{
	...
	if (ptr_Data == NULL)
		return 0; // wrong input formatting
